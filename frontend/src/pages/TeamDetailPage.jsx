...
import TimelineEventItem from '../components/teams/TimelineEventItem';
import { getSortedTimelineItems } from '../utils/teamHistoryUtils';
...
function TeamDetailPage() {
  ...
  // Utilize the testable utility for sorting logic
  const timelineItems = React.useMemo(() => {
    return getSortedTimelineItems(data?.timeline, data?.events);
  }, [data]);
            ...
            {timelineItems.length > 0 ? (
              <div className="timeline-list">
                {timelineItems.map((item, index) => {
                  if (item.type === 'EVENT') {
                    return <TimelineEventItem key={`evt-${item.event_id}`} event={item} />;
                  }

                  // Render ERA (existing logic)
                  const era = item;
                  return (
                    <div key={`era-${index}`} className="timeline-era">
