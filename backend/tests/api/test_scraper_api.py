...
async def test_scraper_start_requires_admin(client: AsyncClient):
    """POST /api/v1/admin/scraper/start requires admin role."""
    response = await client.post("/api/v1/admin/scraper/start")
...
async def test_scraper_start_as_admin(
    ...
    with patch('app.api.admin.scraper.run_scraper_with_logging') as mock:
