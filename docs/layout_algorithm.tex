...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=1.5cm, auto]
        ...
        \node [decision, below of=input] (check) {Check Cache?};
        \node [block, below of=check] (phase1) {Phase 1: Build Chains};
        ...
        \node [block, above of=energy, node distance=2.5cm] (hybrid) {Phase 4: Hybrid Optimization};
        \node [block, above of=hybrid, node distance=2cm] (norm) {Normalization \& Compaction};
        \node [block, above of=norm] (output) {Final positions};

        \path [line] (input) -- (check);
        \path [line] (check) -- node {Miss} (phase1);
        \path [line] (check) -| node[near start] {Hit} (output);
        ...
        \path [line] (energy) -- node {No} (hybrid);
        \path [line] (hybrid) -- (norm);
        \path [line] (norm) -- (output);
    \end{tikzpicture}
    \caption{Layout Algorithm Pipeline}
\end{figure}

\section{Phase 0: Precomputed Optimization}
\textbf{Goal}: Skip expensive client-side calculation for complex families by using cached server-side layouts.

\textbf{Algorithm}:
\begin{enumerate}
    \item Compute \textbf{Family Hash} (SHA-256) based on structural topology (Nodes + Links).
    \item Check backend API for \texttt{PrecomputedLayout} record with this hash.
    \item \textbf{If Found}: Apply cached Y-coordinates directly to chains (O(1) complexity).
    \item \textbf{If Missing}: Proceed to Phase 1 (Standard Layout Algorithm).
\end{enumerate}

\textbf{Complexity Threshold}: Only families with $>20$ chains trigger the cache check. Simpler families are always calculated client-side to avoid network latency.
...
\section{Phase 4: Hybrid Groupwise Optimization}
\textbf{Goal}: Fine-tune the layout by optimizing entire family branches together.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=1.5cm, auto]
        ...
        \node [block] (input) {Output from Phase 3};
        \node [block, below of=input] (group) {Build Groups (Bottom-Up)};
        \node [decision, below of=group, node distance=2.5cm] (rigid) {Try Rigid Move};
        \node [decision, below of=rigid, node distance=2.5cm] (swap) {Try Pairwise Swaps};
        \node [decision, below of=swap, node distance=2.5cm] (anneal) {Try Annealing};
        \node [block, right=2cm of rigid] (apply) {Apply Move};
        
        \path [line] (input) -- (group);
        \path [line] (group) -- (rigid);
        \path [line] (rigid) -- node {No Improved} (swap);
        \path [line] (rigid) -- node {Improved} (apply);
        \path [line] (swap) -- node {No Improved} (anneal);
        \path [line] (swap) -- node {Improved} (apply);
        \path [line] (anneal) -- node {Improved} (apply);
    \end{tikzpicture}
    \caption{Hybrid Optimization Strategy}
\end{figure}

\subsection{1. Build Groups (Bottom-Up)}
Chains are grouped into tightly coupled ``blocks'' based on connectivity.
\begin{itemize}
    \item \textbf{Start}: Leaf nodes or roots (low degree).
    \item \textbf{Grow}: Add connected parents/children using BFS.
    \item \textbf{Stop}: When the group reaches a natural boundary.
\end{itemize}

\subsection{2. Strategy A: Rigid Group Move}
\textbf{Concept}: Move the entire group ($G$) up or down by $\delta$ lanes without changing relative internal positions.
\begin{itemize}
    \item \textbf{Search Space}: $\pm 20$ lanes.
    \item \textbf{Constraint}: No chain in $G$ can collide with a chain outside $G$.
    \item \textbf{Objective}: Find $\delta$ that minimizes total global cost.
    \item \textbf{Why}: Fixes cases where a whole family branch is stuck ``too high'' or ``too low'' relative to its neighbors.
\end{itemize}

\subsection{3. Strategy B: Pairwise Swaps}
\textbf{Concept}: Swap the lanes of any two chains ($A, B$) within the group.
\begin{itemize}
    \item \textbf{Search Space}: All distinct pairs $(A, B)$ in $G$.
    \item \textbf{Constraint}: Swap is valid only if neither chain collides in its new position.
    \item \textbf{Objective}: Fix internal ordering issues (e.g., ``crossing links'').
\end{itemize}

\subsection{4. Strategy C: Simulated Annealing (Fallback)}
\textbf{Concept}: If deterministic methods fail, use stochastic optimization to ``shake'' the group out of a local minimum.
\begin{itemize}
    \item \textbf{Mechanism}: Propose random move, accept if better or probabilistically if worse ($P = e^{-\Delta J / T}$).
    \item \textbf{Cooling}: Temperature decays geometrically ($T_{k+1} = T_k \times 0.95$).
    \item \textbf{Why}: Resolves complex ``tangles'' needing multiple moves to escape.
\end{itemize}
